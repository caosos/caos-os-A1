...
export default function WCWMonitor() {
    ...
    const fetchWCW = async (sid) => {
        ...
        try {
            ...
            setError(err.message || 'Failed to fetch WCW data');
            setWcwData(null);
        } finally {
            setLoading(false);
        }
            ...
            <Card>
                <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Select a session to inspect its token budget usage, injection headroom, and WCW health status.
                    </p>
                </CardHeader>
                <CardContent className="space-y-4">
                    <SessionSelector selectedSessionId={sessionId} onSelectSession={handleSelectSession} />
                    {sessionId && (
                        <Button onClick={() => fetchWCW()} disabled={loading} variant="outline" size="sm">
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle className="truncate">{sessionTitle || wcwData.session_id}</CardTitle>
                                <p className="text-xs font-mono text-muted-foreground mt-1 truncate">{wcwData.session_id}</p>
                            </div>
                            <Badge className={getBadgeColor(wcwData.utilization_pct)}>
                        ...
                        {wcwData.utilization_pct >= 80 && (
                            <div className="p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-red-800 mt-1">This session is approaching the WCW ceiling. Automatic regulation will trigger soon.</p>
                                </div>
                            </div>
                        )}
                        {wcwData.utilization_pct < 60 && (
                            <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-green-800 mt-1">Session has adequate headroom for continued operation.</p>
                                </div>
                            </div>
                        )}
