...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`}</Code>
              ...
              <div className="bg-red-950/30 border border-red-500/20 rounded-lg p-4">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className="text-red-300 font-bold text-sm">TSB-042 — Prompt Budget Governor v1: Char-Budget Enforcement Pre-Inference</span>
                  <Tag label="LIVE ✅" color="green" />
                </div>
                <Code>{`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TSB-042 — PROMPT BUDGET GOVERNOR v1
DATE: 2026-03-15
OWNER: MICHAEL / BASE44
STATUS: LIVE ✅
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🧭 PROBLEM
WCW was measured only AFTER inference (from openaiUsage.prompt_tokens).
There was no pre-flight gate. On long campaign threads + TRH + MBCR + file
attachments, finalMessages could exceed 200k tokens, causing OpenAI errors
(context_length_exceeded) with no graceful recovery — just a 502 INFERENCE_FAILED.

🔒 STOP GATE RESULTS (resolved before implementation)
  TOKENIZER_AVAILABLE: NO
    - No tokenizer lib in repo. tiktoken via npm: adds cold-start weight.
    - Chars/4 estimate is unproven and can be off 2–4× for code/JSON content.
    - Decision: Phase 1 uses CHAR budget (deterministic, no external deps).

  CHAR BUDGET DERIVATION:
    - Worst case: pure code/JSON → 1 token ≈ 2 chars (single-char tokens, punctuation)
    - 182,000 token prompt budget × 2 = 364,000 chars = MAX_PROMPT_CHARS
    - 182k tokens leaves 18k tokens below 200k context (vs 8k completion reserve)
    - Conservative: real mixed content will use 3–4 chars/token → actual token count
      well under 182k at the char cap.

  MESSAGE IDENTITY: Deterministic by position + role + content prefix.
    No fragile name-matching. All prefixes confirmed stable in assembled finalMessages.

  NEXT_TSB_NUMBER: TSB-042 confirmed from prior read.

🏗️ IMPLEMENTATION

applyBudgetGovernor(messages, maxChars) — pure function, inlined per §16.1.
  Input:  finalMessages[] assembled in main handler
  Output: { messages, actions[], over_budget, chars_before, chars_after }

Priority drop order (what gets cut first):
  1. trhMetaMarker      role=system, "EXECUTION_META_TRH:..." prefix — drop entirely
  2. Oldest history     non-injected, non-last messages — drop from front until within budget
  3. MBCR excerpts      role=system, "THREAD RECOVERY..." prefix — truncate to 16k chars
  4. TRH summary        role=assistant, "THREAD SUMMARY..." prefix — truncate to 16k chars
  5. User input         last message — truncate text part to 64k chars (with explicit marker)

System prompt [0] is NEVER touched (protected).

Call-site: inserted between finalMessages assembly and handleInference() call.
  const budgetResult = applyBudgetGovernor(finalMessages, MAX_PROMPT_CHARS);
  // governor logs + emits PipelineEvent on any action
  const governedMessages = budgetResult.messages;
  // handleInference receives governedMessages, not finalMessages

Receipt fields added to response:
  budget_governor: { applied, actions[], chars_before, chars_after }
  Also present in execution_receipt via buildResponsePayload.

Observability: PipelineEvent emitted at BUDGET_GOVERNOR stage with WARN level
  whenever any action is taken.

GOV v1.2 Amendment A compliance:
  hybridMessage was 1015 lines pre-patch (≥300). Change = 1 constant + 1 function
  definition in SECTION 4 + 1 call-site block in SECTION 7. No restructuring.

ROLLBACK:
  Replace \`const inferResult = await handleInference({ ..., finalMessages: governedMessages, ... })\`
  back to \`finalMessages\`. Remove budgetResult/governedMessages lines. Remove applyBudgetGovernor
  function definition. Remove MAX_PROMPT_CHARS constant. Remove budget_governor from buildResponsePayload.
  All in hybridMessage — single file, no new modules created.

PHASE 2 (future, separate TSB):
  When real tokenization is available (tiktoken, or OpenAI token-count endpoint),
  swap MAX_PROMPT_CHARS for MAX_PROMPT_TOKENS and replace countChars() with countTokens().
  Governor structure is identical — only the measurement function changes.

Line count: 891 → 1016 (+125 lines — governor function ~100L + call-site ~15L + constant ~10L)`}</Code>
              </div>
