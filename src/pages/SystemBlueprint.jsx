...
- Calm, direct, architect-level tone`}</Code>
          ...
          <Section title="7. Backend Functions" color="green">
            <Code>{"MAIN PIPELINE:\nhybridMessage - ALL chat flows through here. Auth, memory, history, heuristics, OpenAI call, message save, receipt write.\n\nVOICE / AUDIO:\ntextToSpeech - OpenAI TTS (tts-1-hd). Returns JSON { audio_base64 }. LOCKED. Called via base44.functions.invoke() only.\ntranscribeAudio - Whisper transcription (voice input → text)\ngoogleTextToSpeech - Returns browser Web Speech config (no API call). LOCKED.\n\nTHREAD MANAGEMENT:\ngenerateThreadSummary - summarize a conversation thread (OpenAI call)\n\nRUNTIME AUTHORITY & CONFIGURATION:\nruntimeAuthority - CENTRALIZED SOURCE OF TRUTH (NEW — Mar 2, 2026) Single RUNTIME_AUTHORITY object exported by this module. All other modules import and use this for canonical values. Eliminates duplicated config across system. Contains: model_name, token_limit, platform_name, hosting_platform, backend_runtime, frontend_framework, inference_provider, capabilities, safeguards.\n\nsystemSnapshot - Generate system state snapshot (REFACTORED Mar 2, 2026) Now imports RUNTIME_AUTHORITY instead of hardcoded values. Ensures all snapshots reflect single canonical source.\n\nWEB SEARCH & ARTIFACTS:\nwebSearch - Real web search via Bing API (NEW — Mar 2, 2026) Performs actual external search. Returns structured results with title, url, snippet, publication date. Signature: { query, limit } → { results[], hash }\n\nexternalKnowledgeDetector - Automatic web trigger detection (NEW — Mar 2, 2026) Analyzes user input for time-sensitive keywords (today, latest, current, news, price, weather, stock, etc). Returns: { requires_web: boolean } Enables needs-based automatic web search without user prompt.\n\nartifactLoader - Multi-format artifact processor (NEW — Mar 2, 2026) Handles: base44_file, url (allowlisted), inline_base64. Validates size limits: 5MB per artifact, 10MB total per request. Extracts text from files (up to 200K chars). Returns: structured receipt with hash + metadata. Supports: images (multimodal via OpenAI), text files, PDFs (binary only — no OCR extraction yet).\n\nDIAGNOSTICS / ADMIN:\nsystemHealth - admin health check (auth required)\ndiagnosticSnapshot - full session state snapshot\ndiagnosticRecall - memory recall trace diagnostic\ninspectPipeline - hybridMessage pipeline step trace\ninspectRouting - function routing audit\nquickInspect - fast session state check\npostPatchAudit - post-deploy verification\nprobeReceiptWrite - test DiagnosticReceipt write path\nrunHybridPipeline - test harness for hybridMessage stages\ngenerateSystemsReport - full system state report\n\nMEMORY / ANCHOR:\ntestAnchors - anchor system test\nextractUserPreference - preference extraction utility\npinMemory - pin a memory entry to structured_memory\ndiagnosticRecall - recall path trace\n\nINTEGRATIONS:\ngrokProvider - Grok/xAI API (built, not the primary model)\ncheckGrokModels - list available Grok models\n\nCORE MODULES (functions/core/):\ncontextBuilder, contextLoader, continuousLearning, deterministicExecutor, diagnosticMode, environmentLoader, errorRecovery, executorContract, globalBinGovernance, indexPersistence, indexedSearch, laneIsolation, latencyTracking, memoryAnchors, memoryUpdate, normalize, persistentSearch, presentationSilence, receiptLogger, routeRegistry, sanitizer, selectorEngine, tieredRecall, tokenizer, toolExecutor, unifiedGovernanceGate, wcwBudget, wcwSelfRegulation\n(BUILT — modular governors. Not all are active in hybridMessage v2)\n(See Section 15 for detailed catalogue of: laneIsolation, indexedSearch, continuousLearning, contextLoader, contextBuilder, memoryUpdate, toolExecutor — purpose, design notes, and intended wiring path for each)"}</Code>
          </Section>
          ...
          <Section title="10. Known Issues & Candidate Next Work" color="blue">
            <Code>{"KNOWN:\n- Memory save strips \"that\" from \"remember that my dog...\" → saves \"my dog's name is Biscuit\"\n  (acceptable — content is accurate)\n- Legacy memory_anchors (auto-extracted) are still injected as INFERRED context\n  → could accumulate noise over time → Phase B will replace this\n- hybridMessage is 620 lines — consider splitting into sub-modules\n- pages/Chat.jsx is 1341 lines — ⚠️ FLAGGED FOR REFACTOR (hard limit is 400)\n  Must be broken into sub-components before adding new features\n- Dropdown menu was showing \"Folders\" instead of \"Links\" — FIXED Mar 3, 2026\n- Photos/Files views were both routing to links panel — FIXED Mar 3, 2026\n- Links were not persistently stored (scan-on-load) — FIXED: now stored in UserFile entity\n\nFILE STORAGE ARCHITECTURE (confirmed Mar 3, 2026):\n- ChatInput.jsx uploads files → base44 CDN → saves UserFile record immediately on attach\n  folder_path organized by conversation: /Conversations/{conversationId}\n- Chat.jsx saveToUserFiles() — secondary layer: after AI reply,\n  scans fileUrls and saves any not already stored\n- Chat.jsx extractAndSaveLinks() — scans AI reply text for https:// URLs,\n  saves each as UserFile type='link' (hostname as name)\n- FileManager.jsx reads UserFile entity filtered by type: 'file' | 'photo' | 'link'\n- All three are deletable from the profile panel\n\nCANDIDATE NEXT:\n- REFACTOR pages/Chat.jsx → extract sendMessage, fileHandling, guestMode to sub-modules\n  (this is the highest priority maintenance item — it MUST happen before new features)\n- Add generated file saving: when AI produces a document/code block, offer save to Files\n- Phase B: Typed schema normalization for structured_memory entries\n- Wire hybridMessage to read UserProfile.tone.style\n- Add memory delete/edit capability to MemoryPanel\n- Activate LexicalRule normalization (chaos → CAOS)\n- Thread auto-summary on conversation close"}</Code>
          </Section>
