...
LOCKED FILES — DO NOT TOUCH WITHOUT EXPLICIT OWNER APPROVAL + TSB ENTRY:
  ...
  - components/chat/ttsController.jsx  (LOCKED — CAOS_TTS_CONTROLLER_v1_2026-03-14)
  - components/chat/ttsPrefs.jsx       (LOCKED — CAOS_TTS_PREFS_v1_2026-03-14)
  - components/chat/ttsTextSanitizer.jsx (LOCKED — CAOS_TTS_SANITIZER_v1_2026-03-14)
...
Wait for the owner to tell you what they need.`}</pre>
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-3">
              ...
              <Tag label="Gemini Provider: LIVE ✅" color="green" />
              <Tag label="RIA Tiered Fallback: WIRED ✅" color="green" />
              <Tag label="Response Reviewer: ACTIVE ✅" color="green" />
...
AI:         OpenAI gpt-5.2 (default) | Gemini models/gemini-2.5-flash (selectable)
            Session toggle: Engine menu in ChatHeader → persists to caos_session_provider
            Note: TTS uses tts-1-hd (separate model namespace — see TSB-011)
            Gemini grounding: native Google Search built-in when Gemini is active
...
Key file:   functions/hybridMessage  ← everything runs through here`}</Code>
  ...
  │
  ├─ 9. INFERENCE CALL: provider-routed — OpenAI gpt-5.2 (default) or Gemini 2.5-flash
  │      └─ resilientInference() wrapper: Tier 1 → (Tier 2 future) → Tier 3 local fallback
  │      └─ FF_RIA_INFERENCE_SPINE=false (Tier 2/3 dormant), FF_GROK_PROVIDER_ENABLED=false
  │      └─ POST-INFERENCE: responseReviewer (Gemini Flash policy gate — fail-open)
  │
...
- Calm, direct, architect-level tone`}</Code>
          ...
          <Section title="7. Backend Functions (Active)" color="green">
            <Code>{"MAIN: hybridMessage (LOCKED — spine — CAOS_HYBRID_MESSAGE_SPINE_v4_2026-03-15 + RIA_WRAPPER_v1_2026-03-23)\nVOICE: textToSpeech (LOCKED), transcribeAudio, googleTextToSpeech (LOCKED)\nRUNTIME: runtimeAuthority (centralized source of truth), systemSnapshot\nWEB: webSearch (Bing API), externalKnowledgeDetector, artifactLoader\nDIAGNOSTICS: systemHealth, diagnosticSnapshot, inspectPipeline, quickInspect\nMEMORY: testAnchors, extractUserPreference, pinMemory\nCORE MODULES: contextBuilder, contextLoader, laneIsolation, memoryUpdate,\n  selectorEngine, tieredRecall, toolExecutor, wcwBudget + more\n\nINFERENCE LAYER (TSB-049/050 — Mar 29, 2026):\n  core/geminiInference  — Gemini provider (LOCKED: CAOS_GEMINI_INFERENCE_v1_2026-03-29)\n                          Converts OpenAI message format → Gemini contents format\n                          Native Google Search grounding (tools: [{googleSearch:{}}])\n                          Multi-modal: image_url → inlineData/fileData\n                          Returns: { ok, content, sources[], model_used, usage, grounding_enabled }\n                          Sources appended to reply as markdown **Sources:** section\n  core/responseReviewer — Post-inference policy gate (Gemini Flash reviewer)\n                          Checks reply against policy checklist (no unauthorized writes, no stale content)\n                          Fail-open: never blocks response on reviewer error\n                          Corrected reply used only if reviewer returns clean=false + corrected_reply\n                          Swap model to DeepSeek-R1 on self-hosted migration\n\nTRH v1 (NEW — Mar 8, 2026 — LIVE ✅):\n  threadRehydrate  — 2-stage: freshness check → LLM summarize (gpt-5.2)\n                     Output: THREAD SUMMARY block saved to Message + injected into finalMessages\n\nMBCR v1 (NEW — Mar 8, 2026 — LIVE ✅):\n  getThreadSnippets — READ-ONLY, no LLM, tag+text match, ±2 neighbor expansion\n                      Used by hybridMessage MBCR inline module for same-thread recovery\n\nCTC MEMORY SYSTEM (LIVE ✅):\n  context/threadIndexLoader  — load ThreadIndex, recalc temperature on access\n  context/crossThreadIntent  — detect cross-thread references in user input\n  context/threadHydrator     — load ContextSeed records, update last_hydrated_at\n  context/sanitizer          — filter filler from message spans (STRICT/STANDARD)\n  context/seedCompressor     — compress raw message spans → ARC pack → ContextSeed\n  context/arcAssembler       — assemble ARC_PACK injection block (2000 token budget)\n\nGOVERNANCE (patched Mar 12, 2026):\n  core/runtimeRegistry  — FunctionManifest CRUD (admin-only). Actor ALWAYS auth-derived from base44.auth.me() — caller-provided actor silently ignored. FunctionManifest CLEAN ✅ — 3 entries, real hashes (TSB-034)\n  core/generalInference — P0 TIMEOUT PATCH ✅ (TSB-033): openaiCallWithTimeout (AbortController 55s) replaces unbounded fetch. Typed errors: PROVIDER_TIMEOUT / PROVIDER_HTTP_ERROR / PROVIDER_NETWORK_ERROR. Observability: provider_request_elapsed_ms, provider_timeout_ms, payload_bytes_est. hybridMessage UNTOUCHED.\n  core/repoTool         — OUTPUT_TRUNCATION guard: MAX_LIST_ITEMS=200. Dirs >200 entries return { ok:false, error_code:'OUTPUT_TRUNCATION', retryable:true, hint } instead of silent over-fetch.\n\nEMOJI LAYER (TSB-035 — Mar 13, 2026):\n  core/promptBuilder    — EMOJI LEGEND + ALWAYS-ON rules added (264 lines, under limit). 8 categories,\n                          ~55 semantic anchors. Single source of truth. No duplicates elsewhere.\n                          Usage rules: max 1/header, max 1/cluster (except severity ladders 🔴🟠🟡🟢),\n                          markers only (never decorative), must match legend meanings.\n\nSSE STREAMING (TSB-036 — Mar 13, 2026 — SHIPPED, CURRENTLY DISABLED):\n  streamHybridMessage   — NEW function. Standalone SSE endpoint. Accepts same input as hybridMessage.\n                          Emits: meta / delta / final / error SSE frames. Option A tool handling\n                          (complete tool loop first, stream final answer). hybridMessage UNTOUCHED.\n  ENABLE_STREAMING      — Kill-switch in pages/Chat.jsx (line 334). Currently = false.\n                          Set to true to activate. Instant rollback by setting back to false.\n  MessageContent.jsx    — streaming fast-path added: message.streaming=true → plain text + blinking\n                          cursor (bypasses ReactMarkdown + regex per-delta stutter).\n  MarkdownMessage.jsx   — React.memo() applied for stable re-render on content changes.\n\nTTS EMOJI STRIP (TSB-037 — Mar 13, 2026):\n  ChatBubble + ChatInput + ChatInputReadAloud + ChatBubbleReadAloud — stripEmojis() applied\n  to cleanText pipeline in ALL four TTS path entry points before vocalization."}</Code>
          </Section>
